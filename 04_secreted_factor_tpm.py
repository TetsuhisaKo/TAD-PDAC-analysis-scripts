...
TPM-based sensitivity analysis of selected genes shown in Fig. 3d.

Analysis corresponding to Supplementary Table S8B:
...
The 12 genes shown in Fig. 3d are:
...
if int(np.sum(groups == "TAD")) != 6 or int(np.sum(groups == "non-TAD")) != 30:
    raise ValueError(
        "Group counts do not match the specified cohort "
...
# Optional merge with the primary DESeq2 results.
...
# Arrange to mirror Supplementary Table S8B.
preferred_cols = [
    "Gene",
    "DESeq2_log2FC",
    "DESeq2_adjusted_P",
...
csv_path = OUT_DIR / "Supplementary_Table_S8B_TPM_sensitivity.csv"
xlsx_path = OUT_DIR / "Supplementary_Table_S8B_TPM_sensitivity.xlsx"
